...
public class AutoPlaceBlockSwitch_Left extends CommandGroup {
	public AutoPlaceBlockSwitch_Left(SRXTankDrive drive, Forklift forklift) {
		...
		if (gameData.charAt(0) == 'L') {
			addSequential(drive.new CmdMoveForward(autoLineDistance, 4000, 0.75)); // drive forward to autoline
			...
			addSequential(drive.new CmdInPlaceTurn(90, 0.75, 1500, Direction.RIGHT)); // move to right position(turn
																						// right)
			Log.info("MainGuido(AUTO)", "[2]reached target position");
			// forklift.setState(Forklift.State.SWITCH, Intake.State.OUTTAKE); //push block
			// on to switch
